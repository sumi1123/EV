...
Module VariableModule
    ...
    'ListView相关定义
    Public Const INDEX_PAR = &HFFFF '显示框能够显示的最大行数值
    '系统运行时间相关定义
    Public Running_Hour As Integer = 0 '运行小时
    Public Running_Min As Integer = 0 '运行分钟
    Public Running_Sec As Integer = 0 '运行秒
    Public Running_Sec_Save As Integer = 0 '运行秒
    Public Save_Time_Par As Integer = 43200 '保存间隔时间，12小时保存1次,12h*60m*60s = 43200s
    'CAN滤波设置相关定义
    Public Structure Filter_Struct
        Public FilterMode As Byte
        Public ID1 As UInteger
        Public ID2 As UInteger
        Public AccCode As UInteger
        Public AccMask As UInteger
    End Structure
    Public FilterStruct As Filter_Struct
    '***********************************************************************************************  
    '数据帧信息显示
    Public Function FrmInfoShowForListView(ByVal TimeInterval As Single, ByVal DataDirect As String, ByVal FrameInfo As CANAdapter.CAN_OBJ, ByVal Data As String) As Boolean
        ...
        If FrameInfo.RemoteFlag = 0 Then
            item.SubItems.Add(String.Format("{0:X4}", Data)) '写入帧数据 ‘一段时间后需要清空
            USBCAN_Tool.TextBox1.Text = String.Format("{0:X4}", Data) '写入帧数据
        Else
        ...
        Else
            ...
            For Receive_FrmNum = 0 To FrameNum - 1
                ...
                If DisplayRecFrameFlag = 0 Then
                    USBCAN_Tool.DisplayMessage() '显示接收CAN信息帧
                End If
            Next
        End If
    End Function
